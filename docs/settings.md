...
- **Bluetooth GATT (custom service, characteristics, Air Station TLV):** [`docs/ble-characteristics.md`](ble-characteristics.md). **Air Station** (`0x06`): Wi‑Fi/geo (`0…8`), MQTT (`9…17`), **`TZ` (`18`)**, **`LOG_LEVEL` (`19`)**, **`api_key` (`20`)**, **`startup.toml` one-shots (`21…25`)**. **SSID/password** persist to `settings.toml` but are **not** returned on the read characteristic. **Air Cube** MQTT only via `0x07`. Companion encoder reference: `tools/ble_tlv_reference.py`; capture check: `tools/ble_tlv_verify.py`.
